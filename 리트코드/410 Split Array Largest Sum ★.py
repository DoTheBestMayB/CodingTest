...
# 원인
시간 초과를 해결할 아이디어를 떠올리지 못함

# 내용
최적화 문제를 결정 문제로 바꾸는게 핵심
m개의 배열로 나눴을 때, 배열의 최대 합이 가장 작도록 만들기 -> 최적화
배열의 최대합이 MAX가 되게 m개의 배열로 나눌 수 있는가? -> 결정 문제

배열의 최대합 MAX 최소 값 구하기  -> Parametric Search( Binary Search의 범위 버전 )
배열의 최대합이 MAX가 되게 m개의 배열로 나누기 -> Greedy
